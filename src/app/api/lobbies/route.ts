...
import { lobbies, parties } from "@/db/schema";
  ...
  try {
    return withSchema(async () => {
      ...
      return Response.json({
        lobbies: rows,
        activeCount: Number(recent?.n ?? 0),
      });
    });
  } catch (err) {
    return Response.json(
      { lobbies: [], activeCount: 0, error: describeError(err) },
      { status: 500 },
    );
  }
}
...
export async function POST(req: Request) {
  try {
    return withSchema(async () => {
      ...
      const name =
        String(body.name ?? "Untitled Lobby").slice(0, 40) || "Untitled Lobby";
      const hostName =
        String(body.hostName ?? "Player").slice(0, 20) || "Player";
      const map = MAPS.includes(body.map) ? body.map : "random";
      const teamMode: string = TEAM_MODES.includes(body.teamMode)
        ? body.teamMode
        : "ffa";
      const stackSize = Math.max(
        1,
        Math.min(5, Math.floor(Number(body.stackSize) || 5)),
      );
      const botFill = Math.max(
        0,
        Math.min(11, Math.floor(Number(body.botFill ?? body.botCount) ?? 5)),
      );
      const difficulty = DIFFICULTIES.includes(body.difficulty)
        ? body.difficulty
        : "normal";
      const scoreLimit = SCORE_LIMITS.includes(Number(body.scoreLimit))
        ? Number(body.scoreLimit)
        : 25;
      const timeLimit = TIME_LIMITS.includes(Number(body.timeLimit))
      ...
      // The creator's party remembers this match, so everyone who joined the
      // party code gets a one-click "Join match" button.
      const stackId = String(body.stackId ?? "");
      if (stackId && !stackId.startsWith("solo-")) {
        await db
          .update(parties)
          .set({ lobbyCode: row.code, lobbyName: row.name })
