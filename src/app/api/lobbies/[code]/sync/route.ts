...
import {
  applySync, describeError, drainBotHits, electHost, getLobby, lobbyToNet, mergeEvents,
  reapStale, readSnapshot, toNetPlayer, writeSnapshot,
} from "@/lib/server/lobby-service";
import type { NetBot, NetEvent, SyncRequest, WorldSnapshot } from "@/lib/net/protocol";
...
export async function POST(req: Request, ctx: { params: Promise<{ code: string }> }) {
  try {
    ...
    if (incoming) {
      ...
      const incomingEvents: NetEvent[] = Array.isArray(incoming.events) ? incoming.events : [];
      ...
      const numbered = incomingEvents.map((e) => ({ ...e, seq: e.seq > 0 ? e.seq : ++nextSeq }));
      const merged = mergeEvents({ ...base, events: numbered }, [], ackSeq);
      await writeSnapshot(lobbyCode, { ...base, events: merged.events }, merged.seq);
    }
    ...
    const events = (snap?.events ?? []).filter(
      (e) => e.seq > ackSeq && (e.kind !== "dmg" || !e.to || e.to === playerId),
    );
    ...
    return Response.json({
      ...
      teamScores: [...teamMap.entries()].map(([team, score]) => ({ team, score })),
      serverTime: Date.now(),
      matchOver,
