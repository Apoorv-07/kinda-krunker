...
export async function POST(
  ...
  try {
    return withSchema(async () => {
      ...
      const name = (
        String(body.name ?? "Player").slice(0, 16) || "Player"
      ).trim();
      ...
      const activeCount = roster.filter(
        (r) =>
          Date.now() - new Date(r.lastSeen).getTime() < NET.PLAYER_TIMEOUT_MS,
      ).length;
      ...
      if (!existing) {
        ...
        if (
          stackId !== `solo-${playerId}` &&
          inStack >= Math.min(lobby.stackSize, NET.MAX_STACK)
        ) {
          return Response.json(
            { error: `That stack is full (max ${lobby.stackSize})` },
            { status: 409 },
          );
        }
      }
