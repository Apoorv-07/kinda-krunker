...
export async function electHost(lobbyCode: string): Promise<string | null> {
  ...
  const currentFresh =
    current && now - new Date(current.lastSeen).getTime() < NET.HOST_TIMEOUT_MS;
  ...
  if (current && current.playerId !== next.playerId) {
    await db
      .update(lobbyPlayers)
      .set({ isHost: false })
      .where(eq(lobbyPlayers.id, current.id));
  }
  if (!next.isHost) {
    await db
      .update(lobbyPlayers)
      .set({ isHost: true })
      .where(eq(lobbyPlayers.id, next.id));
    await db
      .update(lobbies)
      .set({ hostPlayerId: next.playerId })
      .where(eq(lobbies.code, lobbyCode));
    next.isHost = true;
  }
...
export async function readSnapshot(
  lobbyCode: string,
): Promise<{ snap: WorldSnapshot | null; eventSeq: number }> {
  const db = getDb();
  const [row] = await db
    .select()
    .from(lobbyState)
    .where(eq(lobbyState.lobbyCode, lobbyCode));
...
export async function applySync(
  ...
  await db
    ...
    .where(
      and(
        eq(lobbyPlayers.lobbyCode, lobbyCode),
        eq(lobbyPlayers.playerId, req.playerId),
      ),
    );
  ...
  if (req.killedBy) {
    ...
    await db
      ...
      .where(
        and(
          eq(lobbyPlayers.lobbyCode, lobbyCode),
          eq(lobbyPlayers.playerId, req.killedBy),
        ),
      );
